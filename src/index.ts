...
._easy-loading p {
     ...
     * */
    _loadingOptions: Record<string, ConfigElementsType> = {
        facebook: {
            style: `
.lds-facebook {
  /* change color here */
...
@keyframes lds-spinner {
  ...
  100% {
     ...
     * @param config
     */
    constructor(config: ConfigType | null | undefined = null) {
     ...
     */
    deletesElement(): void {
        const el: HTMLElement = document.getElementById(this._config.id);
        if (el !== null) {
            el.remove();
        }
        const style: HTMLElement = document.getElementById(this._config.styleId);
        if (style !== null) {
            style.remove();
        }
    }
